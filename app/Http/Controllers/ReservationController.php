...
class ReservationController extends Controller
{
    ...
    public function approve($id)
    {
        ...
        // F l-fonction approve
$reservation->update(['statut' => 'approuve']); // <--- T-akdi hna mn smiya li f DB (statut aw status)
    ...
    public function reject(Request $request, $id)
{
    ...
    $request->validate([
        ...
        'justification.required' => 'La justification du refus est obligatoire.',
        'justification.min' => 'La justification doit contenir au moins 10 caractères.',
        'justification.max' => 'La justification ne peut pas dépasser 500 caractères.',
    ]);
    
    // Update reservation status - UTILISE 'statut' au lieu de 'status'
    $reservation->update([
        'statut' => 'refuse',
    ...
    // Send notification to user with justification
    try {
        $notification = Notification::create([
            'user_id' => $reservation->user_id,
            'message' => "❌ Votre réservation pour la ressource '{$resource->nom}' a été refusée.\n\nRaison: {$request->justification}",
            'lu' => false,
        ]);
        
        \Log::info('Notification created for rejected reservation', [
            'notification_id' => $notification->id,
            'user_id' => $notification->user_id,
            'reservation_id' => $reservation->id
        ]);
    ...
    return redirect()->back()
                     ->with('success', 'Réservation refusée avec justification envoyée à l\'utilisateur');
}
